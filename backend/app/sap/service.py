...
Responsibilities:
  * pick the provider from config (live HTTP/ICF or honest offline Null)
  * sync master data (plants, customers, materials) and cache it
  * merge SAP nodes into the network as routable nodes with
    road-feeder edges to the nearest transport node (port/airport/rail)
  * mirror disruption lifecycle events into ZHEAL_DISRUPTIONS
  * pull approvals made inside SAP back into the local event log
"""
...
# Global Bike + demo-city coordinates. The ABAP side has no geo data for
# plants/customers, so city names are geocoded here. Unknown cities are
# kept in the SAP listing but not merged into the graph.
CITY_GEO: dict[str, tuple[float, float]] = {
    "HAMBURG": (53.5511, 9.9937), "DALLAS": (32.7767, -96.7970),
    "HEIDELBERG": (49.3988, 8.6724), "SAN DIEGO": (32.7157, -117.1611),
    "TORONTO": (43.6532, -79.3832), "MIAMI": (25.7617, -80.1918),
    "SEATTLE": (47.6062, -122.3321), "MEXICO CITY": (19.4326, -99.1332),
    ...
    "ATLANTA": (33.7490, -84.3880), "PORTLAND": (45.5152, -122.6784),
    "VANCOUVER": (49.2827, -123.1207), "MONTREAL": (45.5017, -73.5673),
    "CALGARY": (51.0447, -114.0719), "BANGALORE": (12.9716, 77.5946),
    "MUMBAI": (19.0760, 72.8777), "DELHI": (28.7041, 77.1025),
    "CHENNAI": (13.0827, 80.2707), "PUNE": (18.5204, 73.8567),
    "SHANGHAI": (31.2304, 121.4737), "SINGAPORE": (1.3521, 103.8198),
    "ROTTERDAM": (51.9244, 4.4777), "SUEZ": (29.9668, 32.5498),
    "COLOMBO": (6.9271, 79.8612), "JAKARTA": (-6.2088, 106.8456),
    "DUBAI": (25.2048, 55.2708), "FRANKFURT": (50.1109, 8.6821),
    "MUNICH": (48.1351, 11.5820), "PARIS": (48.8566, 2.3522),
    "MILAN": (45.4642, 9.1900), "MADRID": (40.4168, -3.7038),
...
TRANSPORT_NODE_TYPES = {NodeType.PORT, NodeType.AIRPORT, NodeType.RAIL_HUB, NodeType.WAREHOUSE}
...
class SapService:
    ...
    def _build_provider() -> SapProvider:
        if not settings.sap_base_url:
            return NullSapProvider("S4_BASE_URL not set — SAP bridge offline (seed network in use).")
    ...
    def status(self) -> dict:
        with self._lock:
            return {
                ...
                "plants": sum(1 for n in self._nodes if n.kind == "plant"),
                "customers": sum(1 for n in self._nodes if n.kind == "customer"),
    ...
    def sync(self) -> SapSyncResult:
        ...
        if self._provider.name == "offline":
            self._last_error = "S4_BASE_URL not set — SAP bridge offline."
            return SapSyncResult(
                ok=False, connected=False, provider=self._provider.name,
        ...
        try:
            ...
            return SapSyncResult(
                ok=True, connected=True, provider=self._provider.name,
            ...
            return SapSyncResult(
                ok=False, connected=True, provider=self._provider.name,
                error=str(exc),
            )
    ...
    def merge_into(self, network: Network) -> Network:
        ...
        transport = [
            n for n in nodes.values() if n.type in TRANSPORT_NODE_TYPES and n.status != "offline"
        ]
        if not transport:
            return network
        ...
        for sap in self._nodes:
            ...
            if nid in nodes:
                continue
            point = GeoPoint(lat=geo[0], lon=geo[1])
            node_type = NodeType.FACTORY if sap.kind == "plant" else NodeType.CUSTOMER
            nodes[nid] = Node(
                id=nid,
                name=sap.name,
                type=node_type,
            ...
            dist = haversine_km(point, nearest.location)
    ...
    def mirror_event(self, event: DisruptionEvent, action: str = "created") -> bool:
        ...
        try:
            row = SapDisruptionRow(
                ...
                start_ts=event.start_time.isoformat() if event.start_time else None,
                end_ts=event.expected_end.isoformat() if event.expected_end else None,
                payload_json=json.dumps(event.model_dump(mode="json"), default=str)[:2000],
            )
            if action == "resolved":
                self._provider.resolve_disruption(event.id)
            else:
                self._provider.create_disruption(row)
            with self._lock:
                self._disruptions_mirrored += 1
            return True
        except SapConnectionError:
            return False
        except SapNotConfigured:
            return False
